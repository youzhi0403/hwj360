<template>
  <div class="goodsList-menu">
    ...
    <div class="myMenuSelect" v-show="menuSelectIsShow">
      <menu-select @selectMenu="selectMenu"></menu-select>
    </div>

    <!--遮罩层-->
    <transition name="fade">
      <div class="list-mask" v-show="menuSelectIsShow" @click="hideList">
      </div>
    </transition>
...
export default {
  ...
  methods: {
    ...
    menuShow () {
      console.log('执行了menuShow')
      this.menuSelectIsShow = !this.menuSelectIsShow
    },
    priceSort () {
    ...
    selectMenu (name) {
      ...
      this.showGoods = this.goods.filter((item) => {
        if (item.classification === this.selectedMenu) {
          return item
        }
      })
    },
